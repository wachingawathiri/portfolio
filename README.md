# Data scientist

#### Technical Skills: Python, R, Latex, kobo collect, Microsoft excel, Power BI

## Education
- MSc. Data Science | University of Nairobi (_November 2023 to date_)								       		
- BSc. Business Information Technology	| Dedan Kimathi University of Technology  (_May 2018 to December 2021_)	 			        		

## WORK EXPERIENCE
**Freelance Data Analyst (March 2024 - present)**

●	Developed close to twenty data collection tools using Kobo Collect and Excel for both survey and assessment projects.

●	Conducted comparative data analysis using R, producing recommendations to enhance interventions in projects such as water and sanitation services, and utilization of routine health data.

●	Part of a team that developed a Data Science curriculum in Python for young girls in tertiary institutions.


**Lead Generator – ABSA Bank Kenya (_December 2022 to July 2023_)**

● Sales and marketing of credit facilities, insurance policies and other bank services and products.

● Preparing and delivering presentations on bank products and services to clients such as teachers.

**Payment Agent – Equity Bank Kenya Limited(_March 2022 to December 2022_)**

● Recruitment and activation of payment till numbers and biller codes for business owners.

● Engage with clients during field work on the bank products and relay any enquiries or feedback they may have

## PROJECT

**Content Developer – ENGAGE – University of Nairobi Project.**

●	Part of a team that developed data science curricula for STEM programs, focusing on Python for young girls in tertiary institutions.

●	Performed exploratory data analysis (EDA), data wrangling, and visualization of health-related datasets

●	Implemented machine learning models.

**Water, Sanitation, and Hygiene (WASH) Services**

•	Conducted comparative data analysis to assess WASH access and impact on vulnerable people such as old people, orphans, and people living with disability (PWD), pre and post-intervention.

•	Evaluated factors influencing access to WASH services among the vulnerable population and assessed the impact of inclusive interventions.

•	Highlighted key focus areas that had improvements due to the interventions and generated a report.

**INCIDENCE DATASET – Group project**

● Data cleaning, processing and analysis with R programming and visualization with shiny application.

● Analyzed the 2020 cancer incidences which included the types and the different age groups and genders.

● Gained insights on which age group and gender are more susceptible to cancer and the most common types.

**SALES DATASET – Personal Project**

● Utilized SQL and Microsoft Excel for data cleaning and analysis of the data and Power BI for visualization

● Analyzed the Key performance indicators which include total revenue, total orders and daily trends

● Gained insights on what product was being sold more and the best and worst selling products


### CERTIFICATIONS
MICROSOFT EXCEL
DATA ANALYTICS
DESIGN THINKING
LEADERSHIP TRAINING
CPA SECTION 1
